...
- **Measured Lighthouse baseline** (from Netlify's automatic deploy-preview audit of the current `/` route, this PR, 2026-08-12 — first real data point against the Phase 4 gate, not something I ran manually):
  ...
  **Post-Phase-1 re-measurement** (same Netlify desktop audit, after the purge commits landed): Performance **68 → 75**, Accessibility/Best Practices/SEO/PWA unchanged (96/100/100/60).

  **Correction**: I initially read that 68→75 jump as a real signal from the purge, contrasted with an earlier 68→70 shift I called noise. A *subsequent* commit that touched only `AUDIT.md` (no site code at all) then moved Performance 75→85 — a 10-point swing with nothing in the build to cause it. That disproves the "68→70 is noise, 68→75 is signal" distinction: the run-to-run variance on Netlify's shared-runner desktop Lighthouse is clearly wide enough (at least ~10 points) to account for everything observed so far, including the post-purge jump. **Treat every number in this table as directional only, not a measurement of any specific change**, until Phase 4 runs a proper controlled Lighthouse pass (fixed environment, multiple runs averaged, mobile config as the brief specifies) rather than reading tea leaves from PR-preview bot comments.
